      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom duration-700">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground text-sm max-w-xl">Unified financial node tracking all Naira (₦) inflows and outflows.</p>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-1">
                ...
                <h3 className="text-5xl font-headline font-bold text-white tracking-tighter">
                  {(profile?.walletBalance || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </h3>
              </div>
            </div>
            <div className="pt-8 border-t border-white/5 grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <p className="text-sm font-bold text-emerald-400">
                  ₦{transactions?.filter(t => t.type === 'funding').reduce((sum, t) => sum + t.amount, 0).toLocaleString()}
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-[8px] font-bold uppercase text-muted-foreground tracking-widest">Outflow (Total)</p>
                <p className="text-sm font-bold text-rose-400">
                  ₦{transactions?.filter(t => t.type === 'spending').reduce((sum, t) => sum + t.amount, 0).toLocaleString()}
                </p>
              </div>
            </div>
          </GlassCard>
          ...
          <GlassCard className="lg:col-span-2 p-0 border-white/10 bg-black/20 overflow-hidden flex flex-col min-h-[500px]">
            <div className="p-6 border-b border-white/5 bg-white/5 flex items-center justify-between">
               <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-headline font-bold text-white">Financial History</h3>
               </div>
               <Button variant="outline" size="sm" className="rounded-xl border-white/10 text-[10px] font-bold uppercase tracking-widest">
                  ...
                  {transactions.map((tx) => (
                    <div key={tx.id} className="p-6 flex items-center justify-between hover:bg-white/5 transition-colors group">
                      ...
                      <div className="text-right space-y-1">
                        ...
                        <p className="text-[8px] font-mono text-muted-foreground uppercase opacity-40">REF: {tx.reference?.substring(0, 12)}</p>
                      </div>
                    </div>
                  ))}
                </div>
